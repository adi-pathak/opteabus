function [motor] = GENERATE_motor_efficiency_map(motor)
 ...
 %This function generates a motor efficiency map of an ASM or PSM with 
 %respect to the specified top level design parameters  

 
%% Initialize from input
% Defined top level parameters
motortype = motor.type;     % PSM or ASM
...
% Specific parameter for PSM (fixed!)
Uberlastfaktor = 1.5;      % overload factor
...
nn = 2000; %number of speed data points 
nT = 800; %number of torque data points 
...
elseif strcmp(motortype, "PSM")
    %% Add and delete paths to avoid erronous usage of functions with same name 
    addpath(genpath([base_path '\PSM']));
    ...
    %% Calculation
    [om_M_eta,OM_Vektor,M,m] = g00_Synchronmotorauslegung(P_n,n_n,n_max,U_n,Uberlastfaktor,p); 
...
if motor.type == 'PSM'; P_n = P_n/1000; end %motor power in kW
