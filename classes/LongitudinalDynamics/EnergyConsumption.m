...
%% Vehicle parameters
vehicle.m                 = Vehicle.Unladenmass+occupancy*Vehicle.Passengercapacity*70/100;%Grossvehiclemass ;%Param.mass.adaptedmass;                             % in kg - Use Mass_calculation() regression functions
if Vehicle.Chassis.numberofaxles==3
...
vehicle.A_front         = Vehicle.Body.height/1000 *...
                          Vehicle.Body.width/1000;                          % in m^2 -  Use: "0.85*Width*Height" to estimate it
vehicle.r_tyre          = Vehicle.Chassis.tyrediameter/2000;                                           % in m
vehicle.f_R             = 0.008;                                           % in - Estimation
vehicle.battery_cap     = Vehicle.Battery.capacity;                          % in kWh - Use battery design description
vehicle.auxiliary       = Vehicle.Passengercapacity * 1000 * 0.1575;       % in W - Estimation
vehicle.GEARBOX         = cell(1,4);                                       % initialize empty gearbox struct
vehicle.MOTOR           = cell(1,4);                                       % initialize empty motor struct



switch Vehicle.Powertrain.topology
   case 1
        vehicle.MOTOR{1} = Vehicle.Powertrain.RA1.motor.efficiencymap;
        vehicle.GEARBOX{1}.gear_ratio       =...
           25;%Vehicle.Powertrain.RA1.transmission.ratio; %[Param.motor.FAGearRatio1 Param.motor.FAGearRatio2];
        vehicle.GEARBOX{1}.eff              = 1;
        architecture = 'GM_X';
    case 2
        vehicle.GEARBOX{1}.gear_ratio       =...
             Vehicle.Powertrain.RA1.transmission.ratio;
        vehicle.GEARBOX{1}.eff              = [1 1];
        vehicle.GEARBOX{3}.gear_ratio       =...
            [Vehicle.motor.RAGearRatio1 Vehicle.motor.RAGearRatio2];
    ...
    case 3
        vehicle.GEARBOX{1}.gear_ratio       =...
            [Vehicle.motor.FAGearRatio1 Vehicle.motor.FAGearRatio2];
        vehicle.GEARBOX{1}.eff              = [1 1];
...
[P_el_min, data_R] =...
 ...
 [vmax,amax,grade] =...
     Simulation_acceleration(Vehicle.Grossvehiclemass,vehicle, v_max_sim, t_sim);
...
range = (Vehicle.Battery.capacity/Econs);% Range of vehicle in km
