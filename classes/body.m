classdef body
    ...
    methods
        ...
        function plotbody(obj,handle,vehicle)
            ...
            frontoverhang=vehicle.Body.frontoverhang;
            rearoverhang=vehicle.Body.rearoverhang;
            if obj.doors==1
                %% door
                ...
                position=[length/2-0.2*(height-groundclearance)
                    0
                    (height-groundclearance)/2];
                if frontoverhang<1000 & wheelbase/2+1.5*diameter>frontoverhang+wheelbase/2-(2*0.1*(height-groundclearance))
                    section_housing_end(obj,length,width,height,groundclearance,position,floorheight,frontoverhang,diameter,handle)
                    length= wheelbase/2-diameter/2 -diameter*.1 -doorwidth/2-50;
                    if length>0
                ...
                %% back section
                position=[-obj.length/2+0.2*(height-groundclearance)
                    0
                    (height-groundclearance)/2];
                
                if rearoverhang<1000 & -wheelbase/2-1.5*diameter<-rearoverhang-wheelbase/2+(2*0.1*(height-groundclearance))
                    section_housing_end(obj,length,width,height,groundclearance,position,floorheight,rearoverhang,diameter,handle)
                    length= wheelbase/2-diameter/2 -diameter*.1 -doorwidth/2-50;
                    if length>0
                        position=[(-doorwidth/2-length/2),0,(height-groundclearance)/2];
                        obj.section_window(length,width,height,groundclearance,floorheight,position,handle);
                ...
                %% sections
                % section 1
                length=(obj.length/2-0.2*(height-groundclearance))-(wheelbase/2+1.5*diameter/2);
                if length>0
    ...
    v5=[-l1;0.5* width;-(0.5)*h+0.25*wheeldiameter+50];
   ...
   % fil1=[fil1,[x(end);0; -0.5*height+floorheight]];
    ...
    h2=bottomsidewallheight;%height/3;
    % side right surface
    n=10; % points in the fillet radi
    r=0.05*height;
    ...
    bottomVertices=obj.translate(bottomVertices,[0 0 -0.5*height+h2/2]);
    bot1Vertices=obj.translate(bot1Vertices,[0 0 -0.5*height+h2/2]);
    bot2Vertices=obj.translate(bot2Vertices,[0 0 -0.5*height+h2/2]);
    bot3Vertices=obj.translate(bot3Vertices,[0 0 -0.5*height+h2/2]);
    bot4Vertices=obj.translate(bot4Vertices,[0 0 -0.5*height+h2/2]);
    bot5Vertices=obj.translate(bot5Vertices,[0 0 -0.5*height+h2/2]);
    ...
    function section_housing_end(obj,length,width,height,groundclearance,position,floorheight,overhang,wheeldiameter,handle)
    height=height-groundclearance;
    colr = [1 1 1];
    alph = 0.8;
    %% define profile
    ...
    x= Cx+R*((cos(linspace(pi/2-a0,pi/2,n))));
    z= Cy+R*((sin(linspace(pi/2-a0,pi/2,n))));
    path=[Vn(1) Vn(2) Vn(3);flip(x)' (x)'*0 flip(z)';flip(Vertarcs);flip(fil1(:,1:end-1),2)' ;r 0 -height*0.5];
    [facel,facer,roof,ws,bot,vertices]=sweep(path,profile,-0.5*height+floorheight);
    ...
    r1=0.05*height;
    h=950;
    l1=(2*r)-overhang+wheeldiameter/2 +wheeldiameter*.1;
    l2=(2*r)-overhang-wheeldiameter/2 -wheeldiameter*.1;
    v1=[l1;0.5* width;-(0.5)* height+r1];
    v2=[l1;0.5* width;-(0.5)*height+0.25*wheeldiameter+50];
    v3=[l2;0.5* width;-(0.5)*height+0.25*wheeldiameter+50];
    v4=[l2-50;0.5* width;v2(3)];
    v5=[l2-50;0.5* width;facel(1,3)];
    
    n=10; % points in the fillet radi
    % bottom fillet
    x= (0.5)* width-r*(1-(cos(linspace(0,pi/2,n))));
    y= -(0.5)* h+r1*(1-(sin(linspace(0,pi/2,n))));
    ...
    n=20;
    y1= -(0.5*height)-groundclearance+wheeldiameter+50; % height of wheelhousing
    x1=(2*r)-overhang+0.25*wheeldiameter;
    Vert1=arc2points(0,[v2(1) v2(3)],[x1 y1]);
    
    y2= -(0.5*height)-groundclearance+wheeldiameter+50; % height of wheelhousing
    x2=(2*r)-overhang-0.25*wheeldiameter;
    %  Vert2=arc2points(-pi,[x2 y2],[v3(1) v3(3)]);
    Vert2=Vert1;
    
    Vert2(:,1)=(((2*r)-overhang)*2-Vert1(:,1));
    %x= 1.1*wheeldiameter/2*(cos(linspace(0,pi,n)));
    %y= -(0.5)*h+floorheight+0.9*wheeldiameter/2*(sin(linspace(0,pi,n))); %% would need to modify for large wheel sizes
    hsg1=[[Vert1(:,1)' flip(Vert2(:,1)',2)];0.5* width+0*[1:n];...
        [Vert1(:,2)' flip(Vert2(:,2)',2)]];
    p1=hsg1(:,1)';
    p1(3)=facel(end,end);
    facel=[facel;p1;hsg1';v4';v5'];
    facer=facel;
    facer(:,2)=-facer(:,2);
    rf1=roof(1:20,:);
    rf1(:,1)=facel(end,1);
    roof=[rf1;roof];
    facerf=[facerf;[[221:220+20] flip([221+20:240+20])]]; % update roof faces
    ...
    windShield=patch(handle,'Faces', facews,'Vertices', ws','FaceColor', colr,'FaceAlpha',alph,'LineStyle','none');
        ...
        function vertices=arc2points(a0,c1,c2)
            ...
            % c=[x y];
            %  hold on
            x=rad*cos(linspace(a0,a1,10));
            y=rad*sin(linspace(a0,a1,10));
            x=x+c(1);
            y=y+c(2);
            % plot3(x,x*0,y);
            vertices=[x' y'];
